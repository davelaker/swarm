...
import { getState, updateTask, addTask, appendLog, writeFinding, swarmDir, writeDeploymentInfo, getRoot } from './state/repo.js';
...
function spawnRemediation(state: SwarmState, gateTaskId: string, cfg: ReturnType<typeof getConfig>): void {
  ...
  const recheckTitle =
    gateAgent === 'reviewer' ? `Code re-review of ${fixId}`
    : gateAgent === 'security' ? `Security re-review of ${fixId}`
  ...
  addTask(recheckTask);
  const gateLabel =
    gateAgent === 'reviewer' ? 'Code Reviewer'
    : gateAgent === 'security' ? 'Security'
    : gateAgent;
  appendLog('pm', `${gateLabel} flagged issues in ${gateTaskId} — asking Coder to fix them (${fixId}), then running ${gateLabel} again (${recheckId})`);
  console.log(`  ↳ remediation spawned: ${fixId} (coder fix) → ${recheckId} (${gateAgent} re-review)`);
}
...
async function readFindingMeta(resultRef: string | null): Promise<{ verdict: string; summary: string }> {
...
function createWorktree(taskId: string): string {
  ...
  try { git(['branch', '-D', branch]); } catch { /* no such branch — fine */ }
...
function mergeWorktree(taskId: string): Promise<void> {
  return serialMerge(async () => {
    ...
    try {
      ...
      const detail = (err as { stderr?: Buffer | string }).stderr?.toString().slice(0, 400)
...
function cleanupWorktree(taskId: string, worktreePath: string): void {
  ...
  try { git(['branch', '-D', `swarm/${taskId}`]); } catch { /* already gone */ }
}
...
export async function runLoop(): Promise<LoopResult> {
  ...
  while (iterations++ < MAX_ITERS) {
    ...
    if (state.tasks.every(t => t.status === 'done' || t.status === 'skipped')) {
      ...
      if (skippedCount) {
        ...
        appendLog('pm', `✓ All ${state.tasks.length} task${state.tasks.length === 1 ? '' : 's'} complete — run finished`);
      }
    ...
    const doneIds  = new Set(state.tasks.filter(t => t.status === 'done' || t.status === 'skipped').map(t => t.id));
  ...
  async function dispatchOne(task: Task): Promise<void> {
    ...
    if (task.assignee === 'coder' && task.attempts === 0) {
      ...
      if (deploymentInfo) {
        try { writeDeploymentInfo(deploymentInfo); } catch { /* non-fatal */ }
      }
    }
    ...
    if (isCoder) {
      try {
        ...
        appendLog('pm', `✗ ${task.assignee} [${task.id}] could not create worktree: ${msg.slice(0, 200)}`);
  ...
  async function recoverFromDeadlock(state: SwarmState): Promise<'continue' | LoopResult> {
    const deadlockResult: LoopResult = {
      status: 'deadlock', totalCostUsd: totalCost,
    ...
    const sig = blockedGates.map(t => t.id).sort().join(',');
    ...
    const deterministicFallback = (): 'continue' | LoopResult => {
      let spawned = false;
      for (const t of getState().tasks.filter(x => x.status === 'blocked' && taskGates(x, getState()))) {
      ...
      const detail = blockedGates
        .map(t => `${t.id} (${t.assignee}: "${t.title}")`)
        .join('; ');
      appendLog('pm', `⚑ Deadlock — blocked gate task(s) cannot be resolved automatically: ${detail}. Review the findings and decide how to proceed.`);
    ...
    const blocked = await Promise.all(blockedGates.map(async t => {
    ...
    try {
      ...
      const msg = (err instanceof Error) ? err.message : String(err);
      appendLog('pm', `Negotiator errored (${msg.slice(0, 120)}) — falling back to auto-remediation`);
...
function skipDependents(completedId: string, result: import('./dispatch/index.js').TaskResult): void {
  const isEmptyCoder    = result.artifacts !== undefined && result.artifacts.length === 0;
  const isNoopSecurity  = result.verdict === 'APPROVED' && result.artifacts !== undefined && result.artifacts.length === 0;
