// Generic marketplace agent runner for the api-key driver.
// Runs any hired specialist using their stored system prompt with a standard
...
export async function runMarketplaceAgent(
  ...
  // Build context: prior coder findings + charter
  ...
  if (verbose) console.log(`\n  [${agent.id}] dispatched: "${task.title}"\n`);
  ...
  if (verbose) {
    const icon = ['APPROVED', 'COMPLETE', 'ADVISORY'].includes(verdict.toUpperCase()) ? '✓' : '⚠';
    console.log(`\n  [${agent.id}] ${icon} ${verdict}: ${summary}  cost: $${costUsd.toFixed(4)}\n`);
  }
