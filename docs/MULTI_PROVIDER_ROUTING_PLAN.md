...
| MP-13 | Complete (lint baseline remains) | `a2e973f`; `git diff --check`, core `npm test` (148 passing), UI typecheck/test/build (9 UI tests passing) | Documentation now covers Claude/Codex setup, safe availability detection, per-task routing and overrides, telemetry privacy, and the read-only broker-mediated Codex patch boundary. UI lint remains a pre-existing release limitation: 205 errors / 10 warnings, chiefly required-curly and React effect rules; no unrelated UI code was changed. Verification used installed dependencies only and made no live provider calls or credential reads. |
| MP-14a | Complete | `c39c02f`; typecheck and 150-test suite pass | Catalog separates Codex-specialised models from API-only Sol/Terra/Luna families and records exact effort plus execution transport eligibility. |
| MP-14b | Complete | `f661922`; typecheck and 152-test suite pass | Codex CLI tasks receive `model_reasoning_effort`; invalid, missing, API-only, or unsupported selections fail closed. Live config smoke remains blocked by the local sandbox state database. |
| MP-14c | Complete | `dbabbc0`; core typecheck/154-test suite plus UI typecheck/11-test suite/build pass | Router and UI select executable model/transport/effort combinations and explain the quality/cost/latency trade-off. |
