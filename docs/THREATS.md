...
| **S3** | **localhost dashboard is unauthenticated** — CSRF / DNS-rebinding lets any web page drive the PM; SSE leaks full state. | Loopback-only bind, per-session token, `Origin`/`Host` enforcement from Phase 3. (`UX.md` §3.) | **implemented (Aug 2026)** — `server/request-guard.ts` enforces `Origin`+`Host` on every request (403 otherwise), all CORS headers removed (UI is same-origin in prod and dev), and the roster/task-graph POSTs are schema-validated (`server/validate.ts`). An August 2026 review found the previously claimed state was wrong: the server set `Access-Control-Allow-Origin: *` on 41 responses and had no origin/host check, making the full drive-by chain (plant roster entry → execute → cross-origin SSE leak of the permission `request_id` → self-approve) real. The **per-session token is still not implemented** — acceptable behind the origin guard for localhost, required before any network exposure. |
