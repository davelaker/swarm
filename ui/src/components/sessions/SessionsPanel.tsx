...
function tierColor(tier: string): string {
  switch (tier) {
    case 'bugfix':    return 'var(--green)';
    case 'feature':   return 'var(--blue, #4f9cf9)';
    case 'greenfield':return 'var(--amber)';
    case 'refactor':  return 'var(--tx-3)';
...
function VerdictDots({ pass, fail, total }: { pass: number; fail: number; total: number }) {
  ...
  return (
    <span style={{ display: 'inline-flex', gap: 3, alignItems: 'center' }}>
      {pass  > 0 && <span style={{ fontSize: 10, color: 'var(--green)',  fontFamily: 'var(--mono)' }}>{pass}✓</span>}
      {fail  > 0 && <span style={{ fontSize: 10, color: 'var(--red)',    fontFamily: 'var(--mono)' }}>{fail}✗</span>}
      {other > 0 && <span style={{ fontSize: 10, color: 'var(--tx-3)',   fontFamily: 'var(--mono)' }}>{other}·</span>}
    </span>
  );
}

function SessionRow({ s, active, onLoad }: { s: SessionMeta; active: boolean; onLoad: () => void }) {
  return (
    <button
      onClick={onLoad}
      style={{
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        <span style={{
          ...
          color: tierColor(s.tier), textTransform: 'uppercase', letterSpacing: '0.06em',
          flexShrink: 0,
        }}>{s.tier}</span>
        <span style={{ fontSize: 11, color: 'var(--tx-3)', fontFamily: 'var(--mono)', flexShrink: 0 }}>
          {formatDate(s.savedAt)}
        </span>
      </div>
      <span style={{
        fontSize: 12, color: 'var(--tx-1)', lineHeight: 1.4,
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        ...
        padding: '12px 16px', borderBottom: '1px solid var(--br)', flexShrink: 0,
      }}>
        <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--tx-2)' }}>
          Session History
        </span>
        <button
          onClick={refresh}
          style={{ fontSize: 11, color: 'var(--tx-3)', fontFamily: 'var(--mono)', padding: '2px 6px' }}
          title="Refresh list"
        >↺</button>
      </div>
      ...
      {/* Error */}
      {(error || loadErr) && (
        <div style={{ padding: '10px 16px', fontSize: 12, color: 'var(--red)', fontFamily: 'var(--mono)', borderBottom: '1px solid var(--br)' }}>
          {error ?? loadErr}
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading && sessions.length === 0 && (
          <div style={{ padding: 32, textAlign: 'center', color: 'var(--tx-3)', fontFamily: 'var(--mono)', fontSize: 12 }}>
            loading…
          </div>
        )}
        ...
        {loadingId && (
          <div style={{ padding: '10px 16px', fontSize: 12, color: 'var(--tx-3)', fontFamily: 'var(--mono)', textAlign: 'center' }}>
            loading session…
          </div>
        )}
      </div>
