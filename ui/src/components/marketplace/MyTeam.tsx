...
export function MyTeam({ team, projectName, onToggle, onRemove, onUpgrade, onViewAgent, onBrowse }: MyTeamProps) {
  return (
    <div className="team-wrap">
      ...
      <div className="team-group">
        ...
        {BUILTINS.map(b => {
          ...
          return (
            <div className="trow clickable" key={b.id} onClick={() => onViewAgent(b.id)} title="Click to view prompt">
              ...
              <div className="trow-main">
                ...
                <div className="trow-sub"><RoleChip role={b.role} /></div>
              </div>
      ...
      {/* Hired */}
      <div className="team-group">
        <div className="team-group-h" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span>Hired{projectName ? ` for ${projectName}` : ''} · {team.length}</span>
          {team.length > 0 && (
            <button
              onClick={onBrowse}
              style={{ fontSize: 11.5, color: 'var(--tx-2)', fontFamily: 'var(--mono)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px', borderRadius: 4 }}
        ...
        {team.length === 0 && (
          <div style={{ padding: '24px 16px', textAlign: 'center', border: '1px dashed var(--border-1)', borderRadius: 'var(--r-lg)', marginBottom: 10 }}>
        ...
        {team.map(h => {
          ...
          if (!a) return null;
          return (
            <div className="trow clickable" key={h.id} style={{ opacity: h.enabled ? 1 : 0.6 }} onClick={() => onViewAgent(h.id)} title="Click to view prompt">
              ...
              <div className="trow-main">
                <div className="trow-name">
                  ...
                  {h.upgradeAvailable && (
                    <span className="upgrade-badge" onClick={e => { e.stopPropagation(); onUpgrade(h.id); }}>
                ...
                {h.grantedTools.length === 0 && (h.grantedConnectors?.length ?? 0) === 0
                  ...
                  : <>
                      ...
                      {(h.grantedConnectors?.length ?? 0) > 0 && (
                        Object.entries(
                          (h.grantedConnectors ?? []).reduce<Record<string, number>>((acc, g) => ({
                            ...acc, [g.server]: (acc[g.server] ?? 0) + 1,
                          }), {})
                        ).map(([server, count]) => (
                          <span key={server} className="connector-chip" title={`${count} ${CONNECTOR_BY_ID[server]?.name ?? server} tools`}>
