...
function capabilityLine(sens: string, scope?: string, sqlCategory?: string): string {
  if (sens === 'shell') {
    if (sqlCategory) {
      ...
      if (sqlCategory === 'destructive')
        return 'Can destroy or alter schema — DROP, TRUNCATE, ALTER.';
    }
    if (scope) {
  ...
  if (sens === 'write')
    return scope ? `Can create and modify files in ${scope}.` : 'Can create and modify files.';
...
function mcpCapabilityLine(sens: ConnectorSens, connectorName: string): string {
  if (sens === 'mcp-write')
    return `Can change data in ${connectorName} — not limited to stated use.`;
  ...
  return (
    <div className="perm-seg">
      {(['allow', 'ask', 'deny'] as PermMode[]).map(m => (
        <button
          key={m}
          className={`perm-seg-btn ${mode === m ? `active ${m}` : ''}`}
          onClick={() => onChange(m)}
        >
          {m}
        </button>
...
export function AgentPage({ a, hired, projectName, onClose, onConfirm }: AgentPageProps) {
  ...
  const connectorGrantedCount = Object.values(connectorGrants).reduce(
    (n, tools) => n + Object.values(tools).filter(m => m !== 'deny').length,
    0,
  );

  const hasSensitive =
    a.tools.some(
      t => !t.locked && (t.sens === 'shell' || t.sens === 'write') && grants[t.name] !== 'deny',
    ) ||
    (a.connectors ?? []).some(req => {
      const def = CONNECTOR_BY_ID[req.id];
      return def?.tools.some(
        t =>
          t.sens === 'mcp-write' &&
          req.tools.includes(t.name) &&
          connectorGrants[req.id]?.[t.name] !== 'deny',
      );
    });
        ...
        <div className="ap-nav-center">
          ...
          {projectName && (
            <span style={{ fontSize: 12, color: 'var(--tx-3)' }}>
              · hiring into{' '}
              <strong style={{ color: 'var(--tx-2)', fontWeight: 600 }}>{projectName}</strong>
            </span>
          )}
        </div>
      </div>
        ...
        <div className="ap-col ap-col-left">
          ...
          <div className="dsec">
            <div className="meta-grid">
              ...
              <div className="mg" style={{ flex: 1 }}>
                ...
                <div className="v" style={{ fontFamily: 'var(--sans)', color: 'var(--tx-1)' }}>
                  {a.changelog}
                </div>
              </div>
            </div>
          ...
          <div className="dsec ap-prompt-sec">
            <div className="dsec-label">
              <IconLock size={11} /> Base prompt · read-only
            </div>
        ...
        <div className="ap-col ap-col-right hire-config">
          ...
          <div className="perm-domain perm-domain-machine">
            <div className="perm-domain-head">
              ...
              <div className="perm-domain-desc">
                What the agent can do on this computer. File access is scoped to this project
                directory.
              </div>
            </div>
            {sortedTools.map(t => {
              ...
              return (
                <div
                  key={t.name}
                  className={`grant ${caution ? 'sensitive' : ''} ${t.sens} ${t.locked ? 'locked' : ''}`}
                >
                  <span
                    className={`tool-i ${iconSens}`}
                  ...
                  <div className="grant-main">
                    ...
                    <div className="grant-name">
                      {t.name}
                      {t.sqlCategory ? (
                        <SqlCatTag category={t.sqlCategory} />
                      ) : (
                        <SensTag sens={t.sens} />
                      )}
                    ...
                    <div className="grant-desc">
                      {descPrefix(t.sens, t.locked, t.scope)}
                      {t.desc}
            ...
            <>
              ...
              <div className="perm-domain perm-domain-services">
                <div className="perm-domain-head">
                  ...
                  <div className="perm-domain-desc">
                    Each service is its own separate gate — enabling or disabling access here has no
                    interaction with machine permissions above. Each service you connect is
                    independently controlled.
                  </div>
                </div>
                {stillProbing && (
                  <div className="connector-probing">Checking connection status…</div>
                )}
                {!stillProbing &&
                  (a.connectors ?? []).map(req => {
                    const def = CONNECTOR_BY_ID[req.id];
                    if (!def) return null;
                    const isAvail = (available ?? []).includes(req.id);
                    const reqTools = [...def.tools.filter(t => req.tools.includes(t.name))].sort(
                      (x, y) => (MCP_RISK[y.sens] ?? 99) - (MCP_RISK[x.sens] ?? 99),
                    );
                    const cgrants = connectorGrants[req.id] ?? {};
                    return (
                      <div
                        key={req.id}
                        className={`connector-block ${isAvail ? 'available' : 'unavailable'}`}
                      >
                        ...
                        {isAvail &&
                          reqTools.map(tool => {
                            ...
                            return (
                              <div
                                key={tool.name}
                                className={`grant ${isWrite ? 'sensitive write' : ''}`}
                              >
                                <span
                                  className={`tool-i ${isWrite ? 'write' : 'read'}`}
                                  style={{ flex: '0 0 26px', width: 26, height: 26 }}
                                >
                                  <ToolGlyph sens={isWrite ? 'write' : 'read'} size={13} />
                                </span>
                                <div className="grant-main">
                                  <div className={`grant-capability ${isWrite ? 'write' : ''}`}>
                                    {capability}
                                  </div>
                                  <div className="grant-name">
                                    {tool.name}
                                  ...
                                  <div className="grant-desc">
                                    {descPrefix(tool.sens)}
                                    {tool.desc}
                                  </div>
                                </div>
                                <PermToggle
                                  mode={cmode}
                                  onChange={m => setConnectorMode(req.id, tool.name, m)}
                                />
                              </div>
                            );
                          })}
          ...
          <div className="dsec">
            ...
            <div className="helper">
              Appended after the template's guardrails. Cannot override them.
            </div>
          </div>
