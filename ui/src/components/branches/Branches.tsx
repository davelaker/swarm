...
function PrChip({ pr }: { pr: SwarmBranch['pr'] }) {
  if (!pr) return null;
  const cls = pr.state === 'merged' ? 'pass'
            : pr.state === 'open'   ? 'changes'
            : 'fail';
  return (
    <a
      href={pr.url}
...
function EmptyState() {
  return (
    <div style={{
      ...
      justifyContent: 'center', gap: 12, height: '100%', padding: 40,
      color: 'var(--tx-3)',
    }}>
      <span style={{ fontSize: 32 }}>⎇</span>
      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--tx-2)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        No swarm branches yet
      </div>
      <div style={{ fontSize: 12, color: 'var(--tx-3)', maxWidth: 320, textAlign: 'center', lineHeight: 1.6 }}>
        Branches are created automatically when you run a task with branch mode enabled.
  ...
  return (
    <div style={{
      ...
      <div className="panel-head" style={{ padding: '0 20px', flexShrink: 0 }}>
        ...
        {lastFetched && (
          <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--tx-3)', marginRight: 10 }}>
        ...
        <button
          ...
          disabled={loading}
          style={{
            fontSize: 11, fontFamily: 'var(--mono)', color: loading ? 'var(--tx-3)' : 'var(--blue)',
            background: 'none', border: 'none', cursor: loading ? 'default' : 'pointer', padding: '0 4px',
          }}
        >
        ...
        {open.length > 0 && (
          <section>
            <div style={{
              ...
              color: 'var(--tx-3)', marginBottom: 10,
            }}>
              Open — {open.length}
            </div>
